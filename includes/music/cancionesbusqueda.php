    ...
    if(!isset($_GET['search'])){
	   ...
	   echo "<meta http-equiv = 'REFRESH' content='0;url=music.php'>";        
        ...
        $consulta = 'SELECT * FROM usuarioscanciones AS uc INNER JOIN relaciones AS r ON uc.idcancion = r.idcancion 
                                                           ...
                                                           WHERE uc.usuario="'.$usuario.'" AND (c.nombre="'.$search.'" OR al.nombre="'.$search.'" OR ar.nombre="'.$search.'" OR g.nombre="'.$search.'")';
        ...
        while($fila = sqlite_fetch_array($resultado)){
             ...
             echo 
             '<tr>
                <td><a href="music.php?page=14&search='.$search.'&song='.$fila['c.idcancion'].'"><div>'.$fila['c.nombre'].'</div></a></td>
